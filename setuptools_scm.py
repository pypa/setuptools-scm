...
def version_from_hg(root, cache_file=None):
...
def version_from_git(root, cache_file=None):
...
def version_from_archival(root, cache_file=None):
...
def version_from_sdist_pkginfo(root, cache_file=None):
...
def _extract_version(root, cache_file, guess_next):
    ...
    for method in methods:
        version = method(root=root, cache_file=cache_file)
        if version:
...
def get_version(cache_file=None, root=None, guess_next=True):
    if root is None:
        root = os.getcwd()
