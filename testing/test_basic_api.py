...
from setuptools_scm import git, hg
...
@pytest.mark.parametrize('expected,data', sorted(hg_archival_mapping.items()))
def test_hg_archival_to_version(expected, data):
    version = hg.archival_to_version(data)
...
def test_git_archival_to_version(expected, data):
    version = git.archival_to_version(data)
    assert format_version(
        version,
        version_scheme='guess-next-dev',
        local_scheme='node-and-date') == expected
